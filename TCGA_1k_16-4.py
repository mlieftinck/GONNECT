import pandas as pd
import torch

from src.analysis.training import plot_training_loss
from src.model.build_model import build_model
from src.train.loss import MSE_Soft_Link_Sum, MSE
from src.train.train import make_data_splits, train_with_validation, save_training_losses

if __name__ == "__main__":
    experiment_name = "test_daic_16-4"
    # Model params
    model_type = "dense"
    biologically_informed = "encoder"
    soft_links = False
    activation_fn = torch.nn.ReLU
    loss_fn = MSE_Soft_Link_Sum(alpha=1.0) if soft_links else MSE()
    # GO params
    go_preprocessing = True
    merge_conditions = (1, 30, 50)
    n_go_layers_used = 5
    # Training params
    dataset_name = "TCGA_complete_bp_top1k"
    n_samples = 10000
    batch_size = 100
    n_epochs = 100
    learning_rate = 0.01
    momentum = 0.9
    patience = 5
    device = "cuda"
    save_losses = True
    loss_path = experiment_name
    # Model storage params
    save_weights = False
    save_weights_path = experiment_name
    load_weights = False
    load_weights_path = ""
    # Additional params
    data_split = 0.7
    seed = 1
    dtype = torch.float64
    n_nan_cols = 5

    # Data processing
    data = pd.read_csv(f"data/{dataset_name}.csv.gz", nrows=min(n_samples, 9797),
                       usecols=range(n_nan_cols + 1000), compression="gzip")
    genes = list(data.columns[n_nan_cols:])
    dataloader, trainloader, validationloader, testloader = make_data_splits(data,
                                                                             n_nan_cols,
                                                                             n_samples,
                                                                             batch_size,
                                                                             data_split,
                                                                             seed)
    # Construct model
    model = build_model(model_type,
                        biologically_informed,
                        soft_links,
                        dataset_name,
                        go_preprocessing,
                        merge_conditions,
                        n_go_layers_used,
                        activation_fn,
                        dtype,
                        genes)
    if load_weights:
        model.load_state_dict(
            torch.load(f"out/trained_models/{dataset_name}/{load_weights_path}_model.pt", weights_only=True))
        print(f"\n----- Loaded weights from file ({save_weights_path}_model.pt) -----")

    # Training
    optimizer = torch.optim.SGD(model.parameters(), lr=learning_rate, momentum=momentum)

    epoch_losses = train_with_validation(n_epochs, trainloader, testloader, validationloader, model, optimizer, loss_fn,
                                         patience, device)
    if save_losses:
        save_training_losses(epoch_losses, f"out/trained_models/{dataset_name}/{loss_path}_results.txt")

    if save_weights:
        torch.save(model.state_dict(), f"out/trained_models/{dataset_name}/{save_weights_path}_model.pt")
        print(f"\n----- Saved weights to file ({save_weights_path}_model.pt) -----")

    # plot_training_loss(f"out/trained_models/{dataset_name}/{loss_path}_results.txt")
