...
def save_masks(layers: [GOTerm], merge_conditions, dataset_name, dtype, model_type="dense"):
    ...
    torch.save(encoder.edge_masks, f"../../out/masks/encoder/{merge_conditions}/{dataset_name}_{model_type}_edge_masks.pt")
    torch.save(encoder.proxy_masks, f"../../out/masks/encoder/{merge_conditions}/{dataset_name}_{model_type}_proxy_masks.pt")
    os.makedirs(f"../masks/decoder/{merge_conditions}", exist_ok=True)
    torch.save(decoder.edge_masks, f"../../out/masks/decoder/{merge_conditions}/{dataset_name}_{model_type}_edge_masks.pt")
    torch.save(decoder.proxy_masks, f"../../out/masks/decoder/{merge_conditions}/{dataset_name}_{model_type}_proxy_masks.pt")
...
def load_masks(module, merge_conditions, dataset_name, model_type):
    ...
    if (module == "encoder") or (module == "both"):
        masks.append(torch.load(f"../../out/masks/encoder/{str(merge_conditions)}/{dataset_name}_{model_type}_edge_masks.pt",
                                weights_only=True))
        masks.append(torch.load(f"../../out/masks/encoder/{str(merge_conditions)}/{dataset_name}_{model_type}_proxy_masks.pt",
                                weights_only=True))
    if (module == "decoder") or (module == "both"):
        masks.append(torch.load(f"../../out/masks/decoder/{str(merge_conditions)}/{dataset_name}_{model_type}_edge_masks.pt",
                                weights_only=True))
        masks.append(torch.load(f"../../out/masks/decoder/{str(merge_conditions)}/{dataset_name}_{model_type}_proxy_masks.pt",
                                weights_only=True))
    if len(masks) == 0:
        return None
    return masks
