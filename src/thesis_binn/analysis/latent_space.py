...
if __name__ == '__main__':
    ...
    if go_preprocessing:
        genes = list(
            pd.read_csv(f"{project_folder}/data/{dataset_name}.csv.gz", compression="gzip").columns[n_nan_cols:])
    ...
    model.load_state_dict(
        torch.load(f"{project_folder}/out/trained_models/{dataset_name}/{experiment_name}_model.pt", weights_only=True))
