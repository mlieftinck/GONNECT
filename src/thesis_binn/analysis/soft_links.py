...
def split_weights_per_module(model: Autoencoder):
    """Split encoder and decoder weights into GO and non-GO weights based on edge masks."""
    weights = []
...
def get_top_k_soft_links(soft_links_per_layer: [torch.Tensor], k: int, layer_index, go_dicts: [dict]):
...
def histogram_weights_per_module(module_weights, bin_width=0.01, a=1.0, color=None):
    """Assumes a list of sparse tensors containing the weights from each layer and plots the distribution of their values."""
    values = []
    for i in range(len(module_weights)):
        values += module_weights[i].values()
...
if __name__ == "__main__":
    ...
    model_FC.load_state_dict(torch.load(f"{project_folder}/out/trained_models/AE_3.-1/AE_3.-1.2_none_model.pt", weights_only=True))
    ...
    # Collect weights for correct module of all models to compare
