...
if __name__ == '__main__':
    ...
    experiment_name = "10-4_cpu"
    label = "stage_pathologic_stage"  # patient_id, sample_type, cancer_type, tumor_tissue_site, stage_pathologic_stage
    seed = 42
    n_nan_cols = 5
    colored = True
    ...
    # genes = list(pd.read_csv(f"../../GO_TCGA/{dataset_name}.csv.gz", compression="gzip").columns[n_nan_cols:])
    ...
    model = build_model(model_type, biologically_informed, soft_links, dataset_name, go_preprocessing, merge_conditions,
                        n_go_layers_used, activation_fn, dtype)
    model.load_state_dict(torch.load(f"../trained_models/{dataset_name}/{experiment_name}_model.pt", weights_only=True))
    print("----- COMPLETED: Building model -----")
    print("----- START: Loading data -----")
    dataset = pd.read_csv(f"../../GO_TCGA/{dataset_name}.csv.gz", compression="gzip")
