...
#SBATCH --mem=5GB          # Request memory (MB) per node. Default is 1024MB (1GB). For multiple tasks, specify --mem-per-cpu instead
...
apptainer exec --nv --writable-tmpfs --pwd /opt/app --containall \
	...
	./container_pixi_0.2.1.sif pixi run python -u src/daic_test_AE_2_cpu.py